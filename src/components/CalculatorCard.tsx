...
const CalculatorCard = ({ title, description, icon, image, onSelect }: CalculatorCardProps) => {
  return (
    <Card 
      className="overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 group border-2 hover:border-primary animate-fade-in hover-scale focus-within:ring-4 focus-within:ring-primary/50 focus-within:border-primary"
      role="article"
      aria-label={`כרטיס מחשבון: ${title}`}
    >
      <div className="relative h-56 overflow-hidden" role="img" aria-label={`תמונת נושא: ${title}`}>
        <img 
          src={image} 
          alt={`${title} - תמונת רקע מחשבון`}
      ...
      <CardContent dir="rtl">
        <Button 
          onClick={onSelect} 
          className="w-full bg-gradient-to-l from-primary to-secondary hover:from-secondary hover:to-primary text-white text-xl font-bold py-7 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 focus:ring-4 focus:ring-primary/50 focus:outline-none"
          aria-label={`פתח את ${title}`}
        >
