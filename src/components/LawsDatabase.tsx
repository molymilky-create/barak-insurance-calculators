...
// חוקים
{
  ...
  shortSummary: "המסגרת העיקרית ליחסים בין מבטח למבוטח: כריתת חוזה, חובת גילוי, אי־התאמה, תרמית, תשלום תגמולים, התיישנות ועוד.",
  ...
  shortSummary: "מסדיר את הפיקוח על ענף הביטוח: רישוי, הוראות פיקוח, סמכויות הממונה, הוראות לציבור, חוזרים ועוד.",
  ...
  shortSummary: "מסגרת הפיקוח על קופות גמל וקרנות השתלמות: רישוי גופים מוסדיים, ניהול כספי חוסכים, דוחות, ממשל תאגידי.",
  ...
  title: 'חוק הפיקוח על שירותים פיננסיים (ייעוץ, שיווק ומערכת סליקה פנסיוניים), תשס"ה-2005',
  year: 2005,
  area: "פנסיה / ייעוץ ושיווק",
  shortSummary: "מסדיר רישוי יועצים וסוכני שיווק פנסיוניים, חובות נאמנות וזהירות, הפרדה בין ייעוץ לשיווק ועוד.",
  ...
  title: 'חוק הפיקוח על שירותים פיננסיים (שירותים פיננסיים מוסדרים), תשע"ו-2016',
  year: 2016,
  area: "שירותים פיננסיים חוץ־בנקאיים",
  shortSummary: "מסדיר רישוי ופיקוח על נותני שירותים פיננסיים (אשראי, אשראי חוץ־בנקאי וכו'), סמכויות המפקח וחובות הדיווח.",
...
// חוזרים
{
  ...
  shortSummary: "מסדיר את דרך צירוף מבוטח לביטוח: איסוף מידע רפואי, שאלון בריאות, מסירת הסברים, תיעוד שיחה/פגישה ועוד.",
  ...
  shortSummary: "קובע הוראות לממשק אינטרנטי שבו סוכן יכול לאתר מוצרי ביטוח והסדר הרשאות 'מורשי צפייה' לעובדי הסוכנות.",
  ...
  shortSummary: "אמנת שירות לסוכנים ויועצים: חובת רמת שירות נאותה, זמינות, טיפול בתביעות, תיעוד פניות ועוד (עודכן ב-2022-10-10).",
  ...
  year: 2022,
  area: "שירות לקוחות / התנהלות סוכן",
  shortSummary: "מעדכן את אמנת השירות לסוכנים ויועצים, כולל חיזוק חובות שירות, זמני מענה וחובת תיעוד.",
  ...
  audience: "גוף מוסדי",
  code: "2021-9-5",
  title: "חוזר גופים מוסדיים 2021-9-5 – הצטרפות לקרן פנסיה או לקופת גמל (תיקון)",
  year: 2021,
  area: "פנסיה / גופים מוסדיים",
  shortSummary: "קובע נוסח אחיד לטפסי הצטרפות לקרן פנסיה/קופת גמל וטפסי שינוי – לשיפור בהירות וזכויות העמית.",
...
const LawsDatabase = () => {
  ...
  const filteredLaws = useMemo(() => {
    return lawsData.filter(law => {
      const matchesText = searchText === '' || law.title.toLowerCase().includes(searchText.toLowerCase()) || law.shortSummary.toLowerCase().includes(searchText.toLowerCase()) || law.area.toLowerCase().includes(searchText.toLowerCase()) || law.code && law.code.toLowerCase().includes(searchText.toLowerCase());
  ...
  return <div className="bg-gradient-to-br from-muted/30 to-muted/10 py-16" dir="rtl">
      <div className="container mx-auto opacity-100 px-0">
        <Card className="shadow-2xl border-2">
          ...
          <CardContent className="space-y-8 p-8">
              ...
              <div className="p-6 bg-gradient-to-br from-primary/5 to-secondary/5 rounded-2xl border-2 border-primary/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input id="search-text" placeholder="חפש בכותרת, תיאור או תחום..." value={searchText} onChange={e => setSearchText(e.target.value)} className="text-right h-12 text-base border-2 focus:border-primary" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="year-filter" type="number" placeholder="לדוגמה: 2020" value={yearFilter} onChange={e => setYearFilter(e.target.value)} className="text-right h-12 text-base border-2 focus:border-primary" />
                  </div>
                </div>
            ...
            <div className="space-y-5">
              ...
              {filteredLaws.length === 0 ? <Alert className="border-2 border-muted animate-fade-in">
                ...
                </Alert> : <div className="grid gap-5">
                  {filteredLaws.map((law, index) => <Card key={law.id} className="hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border-2 hover:border-primary/50 animate-fade-in hover-scale" style={{
                animationDelay: `${index * 0.05}s`
              }}>
                      <CardContent className="p-6 px-[5px] mx-0">
                        <div className="space-y-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {law.code && <Badge className="bg-secondary/20 text-secondary-foreground border-2 border-secondary text-base px-4 py-1.5 font-semibold">
                                📋 {law.code}
                              </Badge>}
                            <Badge variant="outline" className="text-base px-4 py-1.5 font-semibold border-2">
                              📅 {law.year}
                          ...
                          {/* Action Button */}
                          <Button variant="default" size="lg" className="w-full bg-gradient-to-l from-primary to-secondary hover:from-secondary hover:to-primary text-white font-bold shadow-lg hover:shadow-xl transition-all duration-300" asChild>
